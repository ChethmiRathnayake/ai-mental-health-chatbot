...
while True:
    ...
    # SHOW SERIAL BPM ON SCREEN
    if serial_monitor.latest_bpm != "--" and serial_monitor.latest_bpm > 0:
        bpm_display = serial_monitor.latest_bpm
    else:
        bpm_display = 0  # No valid BPM, reset to 0

    cv2.putText(frame, f"BPM: {bpm_display}",
                (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                0.9, (0, 255, 255), 2)

    cv2.putText(frame, f"Stress Frames: {stress_count}",
                (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
                0.8, (0, 0, 255), 2)

    cv2.putText(frame, f"Stress Level: {stress_level}",
    ...
    overlay = frame.copy()
    graph_x = frame_width - graph_width - 10  # Top-right corner X
    graph_y = 10  # Top-right corner Y
    ...
    # Draw axes
    cv2.line(overlay, (graph_x, graph_y), (graph_x, graph_y+graph_height), (200, 200, 200), 1)  # Y-axis
    cv2.line(overlay, (graph_x, graph_y+graph_height), (graph_x+graph_width, graph_y+graph_height), (200, 200, 200), 1)  # X-axis
